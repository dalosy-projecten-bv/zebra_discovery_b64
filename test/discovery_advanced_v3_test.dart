...
void main() {
  //This is a real string, received from an actual device, only the version is changed to 3
  final String discoveryB64AdvancedV3 =
  ...
  group('Test advancedV3', () {
    test('Decode an advancedV3 message', () {
      final discovery = Discovery.fromDiscoveryB64(discoveryB64AdvancedV3);

      expect(discovery, isA<DiscoveryAdvancedV3>());

      expect(discovery.discoveryDataMap['DISCOVERY_VER'], '4');
      expect(discovery.discoveryDataMap['ADVANCED_DISCOVERY_VER'], '3');
      ...
      expect(discovery.discoveryDataMap.length, 52);
      expect(discovery.items.length, 51);
      expect(discovery.error, false);

      final json = discovery.toJson();
      final jsonExpected = jsonDecode(
        testResources('discovery_advanced_v3.json'),
      );
