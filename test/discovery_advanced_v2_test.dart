...
void main() {
  //This is a real string, received from an actual device, only the version is changed to 2
  final String discoveryB64AdvancedV2 =
      ":c0cb";

  group('Test advancedV2', () {
    test('Decode an advancedV2 message', () {
      final discovery = Discovery.fromDiscoveryB64(discoveryB64AdvancedV2);

      expect(discovery, isA<DiscoveryAdvancedV2>());

      expect(discovery.discoveryDataMap['DISCOVERY_VER'], '4');
      expect(discovery.discoveryDataMap['ADVANCED_DISCOVERY_VER'], '2');
      ...
      expect(discovery.discoveryDataMap.length, 51);
      expect(discovery.items.length, 50);
      expect(discovery.error, false);

      final json = discovery.toJson();
      final jsonExpected = jsonDecode(
        testResources('discovery_advanced_v2.json'),
      );
